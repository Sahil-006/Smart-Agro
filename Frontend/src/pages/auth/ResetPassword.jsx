    ...
    <div className="min-h-screen py-12 flex items-center justify-center bg-gradient-to-br from-green-50 via-white to-green-100 px-4">
      ...
      >
        <div className="text-center mb-8">
          ...
          <p className="text-gray-500 font-medium">Create a strong password for your account</p>
        </div>

        {success && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mb-6 p-3 bg-green-50 border border-green-100 text-green-700 rounded-xl text-center text-sm font-semibold">
            {success}
          </motion.div>
        )}

        {error && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mb-6 p-3 bg-red-50 border border-red-100 text-red-600 rounded-xl text-center text-sm font-semibold">
            {error}
          </motion.div>
        )}
...
const Rule = ({ ok, label }) => (
  <div className={`flex items-center gap-2 text-[10px] font-bold uppercase tracking-widest transition-all duration-300 ${ok ? "text-green-600" : "text-gray-300"}`}>
    {ok ? (
      <CheckCircle2 size={14} className="text-green-600" />
    ) : (
      <div className="h-3 w-3 rounded-full border-2 border-gray-200" />
    )}
    {label}
  </div>
);
